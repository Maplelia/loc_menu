...
|指令|用途|
|---|---|
|`!!lm list`|显示分组选择（点击分组自动填入 `!!lm group <组>`）|
|`!!lm list <组>`|查看指定分组的坐标列表|
|`!!lm group`|同 `!!lm list`|
|`!!lm group <组>`|同 `!!lm list <组>`|
...
|指令|用途|
|---|---|
|`!!lm add <组> <名称> <x> <y> <z> [维度]`|手动添加坐标点|
|`!!lm add here <组> <名称>`|添加当前玩家位置（分组不存在时自动新建）|
...
|`编号 X 不存在`|编号超出范围|用 `!!lm list <组>` 查看当前编号|
